...
$$M_k = \sum_i A(k,i) p(i), \quad 1 \le k \le m \tag{1}$$

confidently, as if the point-spread function $A(k,i)$ were known exactly, and pretend it
...
information:
$$p(\text{Scene}|\text{Data}, I) = p(\text{Scene}|I) \frac{p(\text{Data}|\text{Scene}, I)}{p(\text{Data}|I)} \tag{2}$$
the likelihood of a scene,
$$p(\text{Data}|\text{Scene}, I) = \exp \left[ -\frac{1}{2\sigma^2} \sum_k (d_k - M_k)^2 \right] = \exp(-\chi^2/2) \tag{3}$$
...
$$p(\text{Scene}|I_0) \propto W = \frac{N!}{N_1! \dots N_n!} \tag{4}$$

One could visualize this by imagining the proverbial team of monkeys making test
...
$$H \to - \sum_i (N_i/N) \log(N_i/N) \tag{5}$$
...
scene itself: $$N_i \pm \sqrt{N_i} \tag{6}$$
...
$$p_i = \sum_{j=1}^n U_{ij} a_j, \quad 1 \le i \le n \tag{8}$$

our data equation $d = Ap + e$ collapses to

$$d_k - e_k = \sum_{j=1}^m V_{kj} D_{jj} a_j, \quad 1 \le k \le m \tag{9}$$

Only the first $m$ expansion coefficients $(a_1 \dots a_m)$ appear; in this
...
$$L(\text{Scene}) = L(a_1 \cdots a_m) = \exp \left\{ -\frac{1}{2\sigma^2} \sum_{j=1}^m \left[D_{jj} (a_j - b_j) \right]^2 \right\} \tag{10}$$
...
of the form:
$$\begin{pmatrix} .50+q & .25-q \\\\ .25-q & q \end{pmatrix} \quad 0 \le q \le .25 \tag{11}$$
...
multiplicity $$W = \frac{N!}{N_{11}! N_{12}! N_{21}! N_{22}!} \tag{12}$$
...
only two solutions:
$$N_{ij} = \begin{pmatrix} 2 & 1 \\\\ 1 & 0 \end{pmatrix}, \quad \begin{pmatrix} 3 & 0 \\\\ 0 & 1 \end{pmatrix} \tag{13}$$
...
\begin{pmatrix} 12 & 0 \\\\ 0 & 4 \end{pmatrix} \tag{14}$$
...
$$H = -(.5+q)\log(.5+q) - 2(.25-q)\log(.25-q) - q \log q \tag{15}$$
...
$$W \sim \exp \left[ -\frac{128N}{9}(q-1/16)^2 \right] \tag{16}$$
which would lead us to the (mean $\pm$ standard deviation) estimate of $q$:
$$q_{est} = \frac{1}{16} (1 \pm 3/\sqrt{N}) \tag{17}$$
...
$$p(N_1 \dots N_n|I) = W(N) \int x_1^{N_1} \dots x_n^{N_n} G(x_1 \dots x_n) dx_1 \dots dx_n \tag{18}$$
where $W(N)$ is the monkey multiplicity factor (4). Normalization for
all $N$ requires that $G$ contain a delta-function:

$$G(x_1 \dots x_n) = \delta(\Sigma x_i - 1) / g(x_1 \dots x_n). \tag{19}$$

Since $g$ need be defined only when $\Sigma x_i=1$, it really depends only on
...
$$p(N_1 \dots N_n|x_1 \dots x_n) = W(N) x_1^{N_1} \dots x_n^{N_n} \tag{20}$$
...
$$G(x_1 \dots x_n) = p(x_1 \dots x_n|I), \tag{21}$$
...
$$p(N_1 \dots N_n|I) = \int d^n x \, p(N_1 \dots N_n|x_1 \dots x_n) p(x_1 \dots x_n|I) \tag{22}$$
...
$$p(D|x_1 \dots x_n) = W(M) x_1^{M_1} \dots x_n^{M_n} \tag{23}$$
where $W(M)$ is the multiplicity factor (4) with $N$'s replaced by $M$'s everywhere. The
posterior distribution is
$$p(x_1 \dots x_n|DI) = p(x_1 \dots x_n|I) \frac{p(D|x_1 \dots x_n)}{p(D|I)} = A/g \, G(x_1 \dots x_n) x_1^{M_1} \dots x_n^{M_n} \tag{24}$$
...
$$p(K_1 \dots K_n|DI) = AW(K) \int G(x_1 \dots x_n) [x_1^{M_1+K_1} \dots x_n^{M_n+K_n}] d^n x \tag{25}$$
...
$$g = A x_1^{k-1} x_2^{k-1} \dots x_n^{k-1} \tag{26}$$
...
$$p(y_{N+1} = i | y_1 \dots y_N, I) = f(N, N_i) \tag{27}$$
...
$$g(x_1 \dots x_n) = \frac{\Gamma(K)}{\Gamma(k_1) \dots \Gamma(k_n)} x_1^{k_1-1} \dots x_n^{k_n-1}. \tag{29}$$

Denote by $I_D$ the prior information leading to (30). Conditional on
$I_D$, the probability of obtaining the data $(N_1 \dots N_n)$ in $N$
observations is given by (19); using (29) and rearranging, we have
$$p(N_1 \dots N_n|I_D) = \frac{N! \Gamma(K)}{\Gamma(N+K)} \cdot \frac{\Gamma(N_1+k_1)}{N_1! \Gamma(k_1)} \cdots \frac{\Gamma(N_n+k_n)}{N_n! \Gamma(k_n)}. \tag{30}$$

Note that the monkey multiplicity factor $W(N)$ is still contained in
(31). For Laplace's prior (all $k_i=1$) it reduces to
$$p(N_1 \dots N_n|I_D) = \frac{N!(n-1)!}{(N+n-1)!} \tag{31}$$
independent of the
...
$$k_i = k a_i, \quad a_i > 0, \quad 1 \le i \le n \tag{34}$$

and let $k \to 0+$, (31) goes into
...
$$p(N_1 \dots N_n|I_D) \to W(N) (k_1/K)^{N_1} \dots (k_n/K)^{N_n} \tag{37}$$
just
...
$$\int G [x_1^{M_1} \dots x_i^{M_i+1} \dots x_n^{M_n}] d^n x \tag{38}$$
but for
...
changes of notation:
$$k_i \to k_i + M_i; \quad N_i=1, \text{ all other } N_j = 0. \tag{39}$$

We find
$$f_i(M, M_i) = \frac{\Gamma(M+K)}{\Gamma(M+K+1)} \frac{\Gamma(M_i+k_i+1)}{\Gamma(M_i+k_i)} = \frac{M_i+k_i}{M+K} \tag{40}$$
...
(40) can be written $$f_i(M, M_i) = \frac{M p_i + K g_i}{M+K}, \tag{41}$$
...
$$p(L_1|DI) = \int_0^1 p(L_1|x) g(x)dx \tag{42}$$
where
$$p(L_1|x) = \frac{L!}{L_1!(L-L_1)!} x^{L_1} (1-x)^{L-L_1} \tag{43}$$
and a
generating function
$$g(x) = \frac{\Gamma(b)}{\Gamma(a)\Gamma(b-a)} x^{a-1} (1-x)^{b-a-1} \tag{44}$$
where $a = (M_1+k_1)$, $b=(M+K)$. The first two factorial moments of
(42) are then
$$\langle L_1 \rangle = L \int_0^1 x g(x) dx = L \frac{a}{b} \tag{45}$$
$$\langle L_1(L_1-1) \rangle = L(L-1) \int_0^1 x^2 g(x) dx = L(L-1) \frac{a(a+1)}{b(b+1)} \tag{46}$$
from which the (mean) $\pm$ (standard deviation) estimate of the number
$L_1$ of unsampled kangaroos with attribute #1 is
$$(L_1)_{est} = L \left[ p \pm \sqrt{\frac{p(1-p)}{M+K+1} \frac{M+K+L}{N^2}} \right] \tag{47}$$
...
$$(N_1/N)_{est} = p + p^\prime \pm \sqrt{\frac{p(1-p)}{M+K+1} \frac{N+K}{N} \frac{N-M}{N}} \tag{48}$$
...
$$L = \log \left[ \frac{\Gamma(N+h)}{N! \Gamma(k)} \right] \approx \log \left[ \frac{(N+h)^{N+h}}{N^N h^h} \right] + \text{const.} \tag{49}$$

where $h = k-(1/2)$. Thus, when $N$ and $k$ are quite different we have
...
$$\log p(N_1 \dots N_n | I_2) \sim - \Sigma N_i \log(N_i/k_i) + \text{const.} \tag{51}$$

and the most probable sample numbers $(N_1 \dots N_n)$ subject to any
...
$$\log p(N_1 \dots N_n|I^\prime_2) \sim \Sigma k_i \log N_i + \text{const.} \tag{52}$$

and the solution will be the one that maximizes this expression, which
...
$$N_{eff} = (3/.77)^2 k = 15.2k = 3.8K \tag{57}$$
...
correct answer is
$$p(B|DI) = .85 \times .2 / (.85 \times .2 + .15 \times .8) = 17/29 \simeq .59 \tag{58}$$
